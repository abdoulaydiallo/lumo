...
export function OrderDetailsDialog({
  ...
  const storeOrder = order?.storeOrders[0]; // Accès au premier storeOrder
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="p-3 bg-muted/10 rounded-lg">
                            <p className="font-medium">{storeOrder.originAddress?.recipient || "Non spécifié"}</p>
                            <p className="text-sm text-muted-foreground">
                              {storeOrder.originAddress?.formattedAddress || "N/A"}
                            </p>
                            {storeOrder.originAddress?.deliveryInstructions && (
                              <p className="text-xs text-gray-600 italic mt-2">
                                <span className="font-medium">Instructions :</span> {storeOrder.originAddress.deliveryInstructions}
                              </p>
                            )}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="p-3 bg-muted/10 rounded-lg">
                            <p className="font-medium">{storeOrder.destinationAddress?.recipient || "Non spécifié"}</p>
                            <p className="text-sm text-muted-foreground">
                              {storeOrder.destinationAddress?.formattedAddress || "N/A"}
                            </p>
                            {storeOrder.destinationAddress?.deliveryInstructions && (
                              <p className="text-xs text-gray-600 italic mt-2">
                                <span className="font-medium">Instructions :</span> {storeOrder.destinationAddress.deliveryInstructions}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        {storeOrder.payment && (
                          <div className="space-y-3">
                            <div className="flex justify-between">
                              ...
                              <Badge
                                variant={storeOrder.payment.status === "paid" ? "default" : "secondary"}
                              >
                                {storeOrder.payment.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium capitalize">{storeOrder.payment.paymentMethod.replace('_', ' ')}</span>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-4">
                        <div className="p-3 bg-muted/50 rounded-lg">
                          ...
                          <p className="font-medium">{storeOrder.items.length}</p>
                        </div>
                        <div className="p-3 bg-muted/50 rounded-lg">
                          <p className="text-sm text-muted-foreground">Quantité totale</p>
                          <p className="font-medium">
                            {storeOrder.items.reduce((sum, item) => sum + item.quantity, 0)}
                          </p>
                        </div>
                        <div className="p-3 bg-muted/50 rounded-lg">
                          <p className="text-sm text-muted-foreground">Valeur totale</p>
                          <p className="font-medium">
                            {storeOrder.items.reduce((sum, item) => sum + (item.price * item.quantity), 0).toLocaleString()} GNF
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border rounded-lg p-6 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="text-sm space-y-1">
                            <p className="font-medium text-gray-900">
                              {storeOrder.originAddress?.recipient || "Non spécifié"}
                            </p>
                            ...
                            {storeOrder.originAddress?.deliveryInstructions && (
                              <p className="text-xs text-gray-600 italic">
                                Instructions : {storeOrder.originAddress.deliveryInstructions}
                              </p>
                            )}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="text-sm space-y-1">
                            <p className="font-medium text-gray-900">
                              {storeOrder.destinationAddress?.recipient || "Non spécifié"}
                            </p>
                            <p className="text-muted-foreground">
                              {storeOrder.destinationAddress?.formattedAddress || "N/A"}
                            </p>
                            {storeOrder.destinationAddress?.deliveryInstructions && (
                              <p className="text-xs text-gray-600 italic">
                                Instructions : {storeOrder.destinationAddress.deliveryInstructions}
                              </p>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="bg-white border rounded-lg p-6 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          ...
                          {storeOrder.deliveryFeeBreakdown && (
                            <div className="mt-1 text-xs text-muted-foreground">
                              <p>Base: {storeOrder.deliveryFeeBreakdown.baseFee.toLocaleString()} GNF</p>
                              <p>Distance: {storeOrder.deliveryFeeBreakdown.distanceFee.toLocaleString()} GNF</p>
                              {storeOrder.deliveryFeeBreakdown.weightSurcharge && (
                                <p>Poids: {storeOrder.deliveryFeeBreakdown.weightSurcharge.toLocaleString()} GNF</p>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-gray-900">
                            {storeOrder.originAddress?.region === storeOrder.destinationAddress?.region
                              ? "Locale"
                              : "Inter-régionale"}
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-4">
                            <div>
                              ...
                              <p className="font-medium capitalize">{storeOrder.payment.paymentMethod.replace('_', ' ')}</p>
                            </div>
                            <div>
                              <p className="text-sm text-muted-foreground">Statut</p>
                              <Badge
                                variant={
                                  storeOrder.payment.status === "paid" ? "default" :
                                  storeOrder.payment.status === "pending" ? "secondary" : "destructive"
                                }
                                className="mt-1"
                              >
                                {storeOrder.payment.status === "paid" ? (
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <p className="font-medium">{formatDateTime(storeOrder.payment.createdAt)}</p>
                            </div>
                            {storeOrder.payment.transactionId && (
                              <div>
                                <p className="text-sm text-muted-foreground">ID de transaction</p>
                                <div className="flex items-center gap-2">
                                  <p className="font-mono font-medium">{storeOrder.payment.transactionId}</p>
                                  <Button
                                    variant="ghost"
                                    size="icon"
                                    className="h-6 w-6"
                                    onClick={() => copyToClipboard(storeOrder.payment!.transactionId!, "ID Transaction")}
                                  >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{storeOrder.payment.amount.toLocaleString()} GNF</span>
                            </div>
                            <Separator />
                            <div className="flex justify-between">
                              ...
                              <svg className="w-full h-full" viewBox="0 0 36 36">
                                ...
                                <path
                                  d="M18 2.0845
                                    ...
                                    a 15.9155 15.9155 0 0 1 0 -31.831"
                                  fill="none"
                                  stroke={financialSummary.total - storeOrder.payment.amount > 0 ? "#ef4444" : "#10b981"}
                                  strokeWidth="3"
                                  strokeDasharray={`${(storeOrder.payment.amount / financialSummary.total) * 100}, 100`}
                                />
                              </svg>
                              <div className="absolute inset-0 flex items-center justify-center flex-col">
                                <span className="text-xs text-muted-foreground">Payé</span>
                                <span className="font-bold">
                                  {Math.round((storeOrder.payment.amount / financialSummary.total) * 100)}%
                                </span>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="bg-white border rounded-lg p-6 shadow-sm">
                        ...
                        <div className="border rounded-lg overflow-hidden">
                          <Table>
                            ...
                            <TableBody>
                              <TableRow>
                                <TableCell>{formatDate(storeOrder.payment.createdAt)}</TableCell>
                                <TableCell className="capitalize">{storeOrder.payment.paymentMethod.replace('_', ' ')}</TableCell>
                                <TableCell>{storeOrder.payment.amount.toLocaleString()} GNF</TableCell>
                                <TableCell>
                                  <Badge variant={
                                    storeOrder.payment.status === "paid" ? "default" :
                                    storeOrder.payment.status === "pending" ? "secondary" : "destructive"
                      ...
                      <div className="bg-white border rounded-lg p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <p className="font-medium">{storeOrder.deliveryFeeBreakdown.baseFee.toLocaleString()} GNF</p>
                          </div>
                          <div>
                            <p className="text-sm text-muted-foreground">Frais distance</p>
                            <p className="font-medium">{storeOrder.deliveryFeeBreakdown.distanceFee.toLocaleString()} GNF</p>
                          </div>
                          {storeOrder.deliveryFeeBreakdown.weightSurcharge && (
                            <div>
                              <p className="text-sm text-muted-foreground">Supplément poids</p>
                              <p className="font-medium">{storeOrder.deliveryFeeBreakdown.weightSurcharge.toLocaleString()} GNF</p>
                            </div>
                          )}
                        </div>
                      </div>
